...
class World:
    ...
    def get_color(self, gps):
        ...
        if x < 0 or x >= im_width or y < 0 or y >= im_width:
            return 'black'
...
print('testing camera.center_at')
test_count = 0
correct_tests = 0
failure = None

with open('testdata/center_at.in', 'r') as f:
    ...
    for i in range(world_count):
        ...
        for j in range(testcases_count):
            ...
            change_lat, change_lon = map(float, f.readline().split())
            cam.center_at(change_lat, change_lon)
