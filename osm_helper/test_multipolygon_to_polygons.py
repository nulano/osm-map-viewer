...
print('testing osm_helper.multipolygon_to_polygons')
test_count = 0
correct_tests = 0
failure = None
...
for element in root:
    if element.tag == 'relation':
        ...
        if td['type'] == 'multipolygon' and 'test' in td:
            fail = None
            result = helper.multipolygon_to_polygons(element)
            ...
            for polygon in result:
                draw.polygon([(x*scale_x + delta_x, y*scale_y + delta_y) for x, y in polygon], fill=None, outline='red')
